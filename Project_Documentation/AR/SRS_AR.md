...
## تحديث الاستقرار التشغيلي (2025-11-09)

- **NFR-STAGE-01**: توفير بيئة تجريبية مستقلة (`staging.example.com`, `api-staging.example.com`) مطابقة لإعداد الإنتاج، تُغذى من فرع `develop` وتدار وفق ما ورد في `Project_Documentation/EN/CIP.md`.  
- **NFR-PERF-01**: يجب أن تحقق صفحة `/dashboard` زمن FCP ≤ 2.5 ثانية و TTI ≤ 4 ثوانٍ على شبكة Fast 3G قبل أي نشر للإنتاج، مع إلزامية تمرير فحص Lighthouse بدرجة ≥ 80.  
- **NFR-CONFIG-01**: الالتزام بمنظومة التهيئة المعتمدة على البيئة (`APP_ENV`، ملفات `.env.production/.env.staging`، معلمات PM2) لتفعيل التبديل الفوري بين الإنتاج والتجارب.  
- **NFR-OBS-01**: حفظ تقارير محلل الحزم (Bundle Analyzer) ونتائج Lighthouse لكل إصدار داخل `docs/perf/` وربطها بلوحة مراقبة Grafana؛ أي تراجع في المؤشرات يمنع إطلاق الإصدار.  
- **NFR-SEC-01**: اعتماد نموذج الرموز المزدوجة (Access/Refresh) مع ملفات تعريف ارتباط HttpOnly وفرض التحقق عبر `class-validator` لجميع DTO الحرجة كما ورد في CIP §3.1.
