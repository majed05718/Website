...
## مواءمة المراحل مع خطة CIP (2025-11-09)

1. **المرحلة 1 — الاستقرار وإعداد البيئات**  
   - نشر إصلاح التهيئة في `Web/src/app/dashboard/layout.tsx` وتفعيل حالة `hydrated` في `useAuthStore`.  
   - إنشاء بنية التجارب (`/var/www/real-estate-dev`) المعتمدة على فرع `develop` وملف `ecosystem.dev.config.js`.  
   - تطبيق التهيئة المعتمدة على البيئة باستخدام `APP_ENV` وملفات `.env.production/.env.staging`.  

2. **المرحلة 2 — تحسين الأداء في بيئة التجارب**  
   - دمج محلل الحزم واختبارات Lighthouse في خط CI، وإيقاف أي دمج يتجاوز ميزانيات FCP/TTI.  
   - تنفيذ الفهارس الجديدة على جداول Supabase (`properties`, `appointments`, `rental_payments`, `payment_alerts`).  
   - إضافة مكونات البث التدريجي (Streaming) وصفحات التحميل (`loading.tsx`) مع تأخير تحديث المرشحات.  

3. **المرحلة 3 — الأمن وسلامة البيانات**  
   - اعتماد نموذج الرموز المزدوجة (Access/Refresh) باستخدام ملفات تعريف ارتباط HttpOnly وترسيخها في `api/src/auth`.  
   - مراجعة تغطية `class-validator` لجميع DTO الحرجة وإضافة اختبارات سلبية.  
  - توحيد سياسات RLS في Supabase لكل بيئة وتوثيق خطوات تعزيز JWT في `docs/security/jwt-hardening.md`.
