...
export async function generateReceipt(
  ...
  // حفظ الحالة
  new (doc as any).saveGraphicsState();

  // تدوير النص 45 درجة
  const centerX = PAGE_SIZES.A4.width / 2;
  const centerY = PAGE_SIZES.A4.height / 2;

  new (doc as any).setGState(new (doc as any).GState({ opacity: 0.3 }));
  ...
  // استعادة الحالة
  new (doc as any).restoreGraphicsState();
...
export async function downloadInvoice(
  ...
  // الخطوة 2: الآن، قم بإنشاء الـ Blob من هذه الصيغة القياسية والآمنة
